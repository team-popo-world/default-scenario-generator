...
def generate_and_update(userId, chain, invest_merged_df, cluster_df, quest_merged_df, shop_merged_df, user_collection, graph_collection):
    try:
        print(f"🚀 분석 중: {userId}")
        ...
        for df_name, df, col in [
            ...
            if col not in df.columns:
                print(f"❗ 오류: {df_name}에 '{col}' 컬럼이 없습니다. (userId={userId})")
                raise KeyError(f"{df_name} missing column {col}")

        invest_df = invest_merged_df[invest_merged_df["userId"] == userId]
        cluster_user_df = cluster_df[cluster_df["userId"] == userId]
        invest_df = pd.merge(invest_df, cluster_user_df, on="userId", how="left")
        quest_df = quest_merged_df[quest_merged_df["child_id"] == userId]
        shop_df = shop_merged_df[shop_merged_df["userId"] == userId] 

        response_user_json = get_response(userId, chain, invest_df, quest_df, shop_df)

        graph_json = get_graph_json(userId, cluster_user_df, quest_df, shop_df)

        # MongoDB에 저장
        save_to_mongo(user_collection, graph_collection, userId, response_user_json, graph_json)

    except Exception as e:
        print(f"❗ 오류 발생 (userId={userId}): {e}")